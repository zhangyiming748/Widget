...
def get_dir(dictionary):
    ...
    for multi in multi_files:
        ...
        # 如果后缀为webm 加入选定列表
        #################如果是隐藏文件的点
        if suffix == 'mkv' and len(prefix) > 0:
            files.append(multi)
        else:
            print('跳过 %s 文件' % multi)
    return files
...
def toLog(total, current, name, status):
    ...
    with open('/home/pi/Desktop/log.txt', 'a+', encoding='utf-8')as f:
...
def deleteDone(path, name):
    target = str(path) + '/' + str(name)
    os.remove(target)


if __name__ == '__main__':
    Source = '/home/pi/Share/work'
    Target = '/home/pi/Share/work'
